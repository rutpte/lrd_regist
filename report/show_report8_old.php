...
$dd = $disp->displaydateS(date("Y-m-d"));
 ...
 $show_date = date('Y-m-d',strtotime($_POST['s_date']));
 $end_date = date('Y-m-d',strtotime($_POST['e_date']));
 $con="and `register_road`.cre_date between '$show_date' and '$end_date'";
 $dd=$disp->displaydateS($show_date)." ถึง วันที่ ".$disp->displaydateS1($end_date);
}
	*/
...
$pdf->SetFont('AngsanaNew', 'B', 12);
$txt = $pdf->conv("ในเขตพื้นที่ $rsN[name_residence] กรมทางหลวงชนบท กระทรวงคมนาคม");
$pdf->Cell(0, 0, $txt, 0, 1, 'C');    
$pdf->Ln(6);
...
$pdf->SetY(40); 
$pdf->SetX(5);
$txt = $pdf->conv("ชื่อ ทชจ.");
$pdf->Cell(24,20,$txt,1,0,'C',true);

$pdf->SetY(40); 
$pdf->SetX(29);
$txt = $pdf->conv("สายทางในความ");
$pdf->Cell(14,5,$txt,'LTR',0,'C',true);

$pdf->SetY(45); 
$pdf->SetX(29);
$txt = $pdf->conv("รับผิดชอบทั้งหมด");
...
$pdf->SetY(45); 
$pdf->SetX(225);
$txt = $pdf->conv("รวมทะเบียน(2)");
$pdf->Cell(14,5,$txt,'1',0,'C',true);

$pdf->SetY(45); 
$pdf->SetX(239);
$txt = $pdf->conv("รวมทะเบียน(1)+(2)");
$pdf->Cell(14,5,$txt,'1',0,'C',true);
...
$sql="SELECT `province`.id_province,`province`.name_province
FROM
  `province`
  INNER JOIN `municipality` ON (`province`.`id_province` = `municipality`.`id_province`)
  INNER JOIN `register_road` ON (`municipality`.`id_mun` = `register_road`.`id_mun`) where  `register_road`.id_regis_detail!=0  and `province`.id_residence='$id_residence' 
   GROUP BY `province`.id_province
...
$txt = $pdf->conv("$rs[name_province]");
$pdf->Cell(24,5,$txt,1,0,'L',false);

$pdf->SetFont('AngsanaNew', 'B', 7);

if($sum_amount_way!=""){
$sum_amount_way = number_format($sum_amount_way,0,'.',','); 
}else{
$sum_amount_way = "-";
}
$txt = $pdf->conv("$sum_amount_way");
...
$perClass = number_format($perClass,2,'.',','); 
$txt = $pdf->conv("$perClass%");
$pdf->Cell(7,5,$txt,1,0,'C',false);
...
$sum_amount_wayS = number_format($sum_amount_wayS,0,'.',','); 
$txt = $pdf->conv("$sum_amount_wayS");
...
$sum_perClass = number_format($sum_perClass,2,'.',','); 
$txt = $pdf->conv("$sum_perClass%");
$pdf->Cell(7,10,$txt,1,0,'C',true);
