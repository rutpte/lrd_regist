...
$dd = $disp->displaydateS(date("Y-m-d"));
	...
	$s_date1=$_POST['s_date'];
		$s_date = implode("-",array_reverse($xx=explode("-",$s_date1)));
	$e_date1=$_POST['e_date'];
	$e_date = implode("-",array_reverse($xx=explode("-",$e_date1)));
	$d1 = $s_date1;
	$d2 = $e_date1;
...
$pdf->SetFont('AngsanaNew', 'B', 12);
$txt = $pdf->conv("ในเขตพื่นที่สำนักทางหลวงชนบทที่ 1-18 กรมทางหลวงชนบท กระทรวงคมนาคม");
$pdf->Cell(0, 0, $txt, 0, 1, 'C');    
$pdf->Ln(6);
...
WHERE `register_road`.id_regis_detail !=0
AND `residence`.`id_residence`='$rs[id_residence]'
GROUP BY `register_road`.id_mun
...
if($numP3_1!=""){
$numP6_1 = number_format($numP3_1,3,'.',','); 
}else{
$numP3_1 = "-";
}
...
if($class4_1!=""){
$class6_1 = number_format($class4_1,0,'.',','); 
}else{
$class4_1 = "-";
}
