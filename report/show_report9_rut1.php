...
/*$sqlSb="SELECT residence.id_residence,residence.name_residence,way.id_mun
FROM
  residence
  ...
  INNER JOIN way ON (municipality.id_mun = way.id_mun) where waywaywaywayway.id_regis_detail!=0  GROUP BY residence.id_residence order by residence.id_residence asc ";
 $resultSb=$db->query($sqlSb);
   $num_r=$db->num_rows($resultSb);*/
/*    if($num_r==0){
  $TTD="ไม่มีข้อมูลระหว่างวันที่ $dd";?>	  
    <script>alert('<? echo iconv('UTF-8', 'TIS-620',$TTD);?>');	
  window.open('','_self');
window.close() ;</script> 
...
$pdf->SetFont('AngsanaNew', 'B', 12);
$txt = $pdf->conv("ในเขตพื้นที่สำนักทางหลวงชนบทที่ 1-18 กรมทางหลวงชนบท กระทรวงคมนาคม");
$pdf->Cell(0, 0, $txt, 0, 1, 'C');    
$pdf->Ln(6);
...
$pdf->SetY(40); 
$pdf->SetX(225);
$txt = $pdf->conv("จำนวนระยะทางใน");
$pdf->Cell(14,5,$txt,'LTR',0,'C',true);

$pdf->SetFont('AngsanaNew', 'B', 6);
$pdf->SetY(45); 
$pdf->SetX(225);
$txt = $pdf->conv("ความรับผิดชอบทั้งหมด");
$pdf->Cell(14,5,$txt,'LBR',0,'C',true);
...
$pdf->Cell(25,5,$txt,'1',0,'C',true);

$pdf->SetY(45); 
$pdf->SetX(264);
$txt = $pdf->conv("คิดเป็นร้อยละของ");
$pdf->Cell(25,9,$txt,'LTR',0,'C',true);
...
$pdf->Cell(14,10,$txt,'LTR',0,'C',true);


$pdf->SetY(50); 
$pdf->SetX(239);
$txt = $pdf->conv("จำนวน");
$pdf->Cell(12,5,$txt,'LTR',0,'C',true);

$pdf->SetY(50); 
$pdf->SetX(251);
$txt = $pdf->conv("ระยะทาง");
$pdf->Cell(13,5,$txt,'LTR',0,'C',true);

$pdf->SetY(52); 
$pdf->SetX(264);
$txt = $pdf->conv("ระยะทาง (กม.)");
$pdf->Cell(25,8,$txt,'LBR',0,'C',true);
...
$pdf->SetX(239);
$txt = $pdf->conv("สายทาง");
$pdf->Cell(12,5,$txt,'LBR',0,'C',true);

$pdf->SetY(55); 
$pdf->SetX(251);
$txt = $pdf->conv("(กม.)");
$pdf->Cell(13,5,$txt,'LBR',1,'C',true);
...
FROM
  ...
  GROUP BY residence.id_residence,residence.name_residence
  order by residence.id_residence  asc"; //new  GROUP BY residence.id_residence,residence.name_residence
  $result=$db->query($sql);
$sqlSa1="SELECT  
sum(annual_way.amount_phase_way) as sum_amount_phase_ways
FROM annual_way where  list_year!='' and  list_year='$_GET[list_year]' 
";
...
WHERE  residence.id_residence='$rs[id_residence]' and  list_year='$_GET[list_year]' 
GROUP BY residence.id_residence
ORDER BY residence.id_residence ASC ";  //insert new GROUP BY residence.id_residence
...
$txt = $pdf->conv("$rs[name_residence]");
...
if($sum_amount_phase_way!=""){
$sum_amount_phase_way = number_format($sum_amount_phase_way,3,'.',','); 
}else{
$sum_amount_phase_way = "-";
}
$txt = $pdf->conv("$sum_amount_phase_way");
$pdf->Cell(14,5,$txt,1,0,'C',false);
...
$pdf->Cell(13,5,$txt,1,0,'C',false);



$perNum = number_format($perNum,2,'.',','); 
$txt = $pdf->conv("$perNum%");
$pdf->Cell(25,5,$txt,1,1,'C',false);
...
$sum_amount_phase_wayS = number_format($sum_amount_phase_wayS,3,'.',','); 
$txt = $pdf->conv("$sum_amount_phase_wayS");
$pdf->Cell(14,10,$txt,1,0,'C',true);
...
$sum_perNum = number_format($sum_perNum,2,'.',','); 
$txt = $pdf->conv("$sum_perNum%");
$pdf->Cell(25,10,$txt,1,1,'C',true);
