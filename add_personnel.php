...
<? //หน้า add_personel.php จะมีหน้าที่ aad user ใหม่ และรวม การแก้ไข (update) และลบ (del)เข้าไปด้วย จะตรวจสอบจากค่า proc ที่ส่งมา ถ้าไม่มีก็แสดงว่าแอด ?>
...
if($proc==""){
$proc="add";
$sqlS="select id_personnel from personnel where type_personnel='2' ";
$resultS=$db->query($sqlS);
$rsS=$db->fetch_array($resultS);
$numS=$db->num_rows($resultS);
if($numS==0){
$username_per="localroaddev1";     //username ที่ฟิกค่าให้เลยตอน เพิ่มใหม่
}else{

$rn=$numS+1;
$username_per="localroaddev".$rn;       //บวกค่า 1 เข้าไปหลังจากนับเสร็จ
}

////////////////////////////////////////////////////////////////////////////

}else if($proc=="edit"){
$sql="select * from personnel where id_personnel='$id_personnel' ";  //$id_personnel คือค่าที่ get มา
$result=$db->query($sql);
$rs=$db->fetch_array($result);
$username_per=$rs['username_per'];    //fetch มาแต่ username
}
?>
...
<? ////////////////////////////////////////////////////////////////////////////////////?>
<form id="form1" name="form1" method="post" action="manage_per_proc.php">
...
<table width="50%" border="1" cellpadding="0" cellspacing="0" bordercolor="#CCCC33">
  ...
  <tr>
    ...
    <td align="left" bgcolor="#99CC99" class="th11bred"><span id="sprytextfield3"><input name="name_personnel" type="text"  id="name_personnel" value="<? echo $rs['name_personnel'];?>" style="width:140px" <? if($_SESSION['LOGTYPE']==2){?> readonly=""<? }?>/></span></td>
  </tr>
  <tr>
