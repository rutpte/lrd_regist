...
INNER JOIN attachment ON way.way_id=attachment.record_ref_id
INNER JOIN lrd_attachment ON attachment.attach_id=lrd_attachment.attach_id
where way.way_id='$way_id' and lrd_attach_type='F_T2'";  ###########
  $result=$db->query($sql);
...
$folder_file_type=$rs['attach_type'];  //var_dump($folder_file_type);exit;
...
#-----------------------------------------------------------------------------
 ...
 $shH=$sa[1]+300;
       ...
       $shH=$sa[1]+100;
		...
		echo "<script>
				window.location.href('download_file.php?way_id=$way_id')</script>";
}  */
 ...
 $type_file=explode(".",$rs['filename_ref']);     
  if($type_file[1]!="pdf"){
   ...
   $file_for_resize=$path_server."/".$file_get;
	...
	/*	echo "<script>
					//window.location.href('download_file.php?way_id=$way_id')
...
<div align="center"><br/>
 ...
 <!--a href="download_file.php?way_id=<? echo $way_id;?>"> ดาวน์โหลด</a-->
...
<table width="601" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="">
        ...
        <td class="th14bgray"  align="center">ทชจ.&nbsp;<? echo $rs['province_name'];?>&nbsp;&nbsp;<? echo $rs['name_residence']?>
	     ...
	     <tr>
        <td class="th14bgray"  align="center">หน่วยงาน&nbsp;<? echo $rs['orgc_name'];?>  </td>
      </tr>
      <tr>
...
?>
  ...
  <tr>
    <!--td align="center"><? if($rs['file_t2']!=""){?><img src="<? echo $file_for_resize;?>"  /><? }else{?><img src="image/no_image.jpg"  /><? }?></td-->
